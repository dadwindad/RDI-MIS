...
const KanbanBoard = ({ currentUser }) => {
  ...
  const fetchBoardData = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:3003/api/smart-office/documents/board', {
        headers: {
  ...
  const handleDrop = async (e, targetStatus) => {
    ...
    try {
      const response = await fetch(`http://localhost:3003/api/smart-office/documents/${cardId}/status`, {
        method: 'PATCH',
        headers: {
                          ...
                          <button 
                            onClick={(e) => { 
                              ...
                              setConfirmInfo({
                                ...
                                onConfirm: async () => {
                                  try {
                                    const response = await fetch(`http://localhost:3003/api/documents/${doc.id}`, {
                                      method: 'DELETE',
                                      headers: {
                                        'Authorization': `Bearer ${generateMockToken()}`
                                      }
            ...
            <div className="calendar-modal-footer">
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    const response = await fetch(`http://localhost:3003/api/documents/${editingDoc.id}`, {
                      method: 'PUT',
                      headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${generateMockToken()}`
