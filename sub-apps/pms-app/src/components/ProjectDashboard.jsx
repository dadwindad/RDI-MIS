...
const ProjectDashboard = () => {
  ...
  const fetchProjects = async () => {
    try {
      const res = await fetch('/rdi_mis/api/pms/projects', {
  ...
  const fetchTransactions = async (projectId) => {
    try {
      const res = await fetch(`/rdi_mis/api/pms/projects/${projectId}/transactions`, {
        headers: { 'Authorization': `Bearer ${getAuthToken()}` }
      });
      if (res.ok) {
  ...
  const handleDeleteTransaction = async (txId) => {
    ...
    try {
      const res = await fetch(`/rdi_mis/api/pms/transactions/${txId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${getAuthToken()}` }
      });
      if (res.ok) {
        fetchTransactions(selectedProject.id);
        const projectsRes = await fetch('/rdi_mis/api/pms/projects', { headers: { 'Authorization': `Bearer ${getAuthToken()}` } });
  ...
  const fetchFundTypes = async () => {
    try {
      const res = await fetch('/rdi_mis/api/pms/fund-types', {
        headers: { 'Authorization': `Bearer ${getAuthToken()}` }
      });
      if (res.ok) {
  ...
  const handleSaveProject = async (e) => {
    e.preventDefault();
    try {
      const url = isEditing ? `/rdi_mis/api/pms/projects/${formData.id}` : '/rdi_mis/api/pms/projects';
  ...
  const handleActionSubmit = async (e) => {
    ...
    if (type === 'ATTACH') {
      ...
      url = `/rdi_mis/api/pms/projects/${id}/close`;
      payload = { document_base64: data.document_base64, document_name: data.document_name, document_url: data.document_url };
    } else if (type === 'EDIT_TRANSACTION') {
      url = `/rdi_mis/api/pms/transactions/${data.id}`;
  ...
  const handleRemoveDocument = async (doc_type, url) => {
    ...
    try {
      const res = await fetch(`/rdi_mis/api/pms/projects/${selectedProject.id}/remove-document`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${getAuthToken()}`, 'Content-Type': 'application/json' },
  ...
  const approveProject = async (id) => {
    if (window.confirm('Approve this project?')) {
      await fetch(`/rdi_mis/api/pms/projects/${id}/status`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${getAuthToken()}`, 'Content-Type': 'application/json' },
                ...
                {fundTypes.map(ft => (
                  <div key={ft.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem', backgroundColor: 'var(--bg-secondary)', borderRadius: '0.25rem' }}>
                    ...
                    <button onClick={async () => {
                      ...
                      const res = await fetch(`/rdi_mis/api/pms/fund-types/${ft.id}`, {
                        method: 'DELETE',
                        headers: { 'Authorization': `Bearer ${getAuthToken()}` }
                      });
                      if (res.ok) {
