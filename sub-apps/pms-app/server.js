...
const handleBase64Upload = (document_base64, document_name, uploader_name, activity_name) => {
  if (document_base64 && document_name) {
    ...
    return 'http://localhost:3801/storage/' + finalName;
  }
  return null;
};
  ...
  db.get(`SELECT ${column} FROM projects WHERE id = ?`, [id], (err, row) => {
    ...
    db.run(`UPDATE projects SET ${column} = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [newValue, id], async function (err2) {
      ...
      try {
        ...
        await fetch(`http://localhost:3801/api/storage/files/${filename}`, { method: 'DELETE' });
      } catch (e) {
...
app.get('/api/pms/aggregated-data', requireAuth, async (req, res) => {
  try {
    ...
    let activities = [];
    try {
      const soRes = await fetch('http://localhost:3803/api/activities', {
